...
export default function SessionDetailPage() {
  ...
  // Stream approval requests (only when running)
  const {
    pendingApprovals,
    respond: respondToApproval,
  } = useApprovalStream(session?.status === 'running' ? sessionId : null);
  ...
  useEffect(() => {
    ...
    const fetchLogsForProcess = async (process: ExecutionProcess) => {
      // Skip if already fetched or currently running
    ...
    // Fetch logs for all completed processes
    ...
    // Add running process to turns (with empty logs - will use live stream)
    if (runningProcess) {
      setConversationTurns((prev) => {
  ...
  // Auto-scroll when new content arrives (including live log updates)
  useEffect(() => {
    if (!userScrolledUp && messagesContainerRef.current) {
      // Use scrollTo for smoother continuous scrolling during live updates
  ...
  const handleFollowUp = async () => {
    ...
    try {
      ...
      setFollowUpPrompt('');
      // Clear the fetched set so we refetch after new process is created
  ...
  const canFollowUp = !isRunning && session.status === 'completed' && session.agentSessionId;
      ...
      >
        <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
          ...
          {sortedTurns.map((turn, index) => {
            // Only show live logs if this is the running process AND the WebSocket is connected to the correct process
            ...
            <textarea
              ...
              placeholder={
                isRunning
                  ...
                  : session.status === 'completed' && !session.agentSessionId
...
function InlineApprovalCard({
  ...
  // Extract a summary for collapsed view
  const getSummary = () => {
...
function ConversationTurnView({
  ...
  // Use live logs if streaming, otherwise historical
  const displayLogs = isLive ? liveLogs : logs;

  // Auto-scroll for live logs within this container
  useEffect(() => {
